...
class XTTSEventHandler(AsyncEventHandler):
    def __init__(self, wyoming_info: Info, engine: XTTSEngine, voices_path: Path, language_fallback: str | None, no_detect_language: bool, min_segment_chars: int, *args: Any, **kwargs: Any) -> None:
        ...
        self._streaming = StreamingHandler(self, engine, voices_path, language_fallback, no_detect_language, min_segment_chars)
    ...
    async def handle_event(self, event: Event) -> bool:
        ...
        if SynthesizeStart.is_type(event.type):
            try: await self._streaming.handle_start(SynthesizeStart.from_event(event))
            except Exception as err: await self._streaming.handle_error(err)
            return True
        if SynthesizeChunk.is_type(event.type):
            try: await self._streaming.handle_chunk(SynthesizeChunk.from_event(event))
            except Exception as err: await self._streaming.handle_error(err)
            return True
        if SynthesizeStop.is_type(event.type):
            try: await self._streaming.handle_stop()
            except Exception as err: await self._streaming.handle_error(err)
            return True
        if Synthesize.is_type(event.type):
            if self._streaming.has_active_session: return True
            try: await self._handle_synthesize(Synthesize.from_event(event))
    ...
    async def _handle_synthesize(self, synthesize: Synthesize) -> None:
        ...
        for segment in segmenter.add_chunk(text):
            ...
            if first_audio is None: first_audio = result
